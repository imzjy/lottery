...
namespace BuyLottery.DataAccess
{
    class SqliteHelper
    {
        ...
        {
            _connectionString = ConfigurationSettings.AppSettings["repo"];
            if (string.IsNullOrWhiteSpace(_connectionString))
            {
                _connectionString = string.Format(@"data source={0}data\repo", AppDomain.CurrentDomain.BaseDirectory);
            }
            if(!File.Exists(_connectionString.Split('=')[1]))
            {
                throw new ApplicationException("Ticket repositry file not found");
            }
